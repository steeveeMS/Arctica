...
client.on('message', message => {
    ...
    client.channels.get(config.textlog).send(`**${message.author.tag}:** ${message.content} | *sent in* ${message.channel} at ${moment().format('LLLL')}`)
...
//VOICECHANNEL JOIN+LEAVE DETECTION w/o switching
client.on('voiceStateUpdate', (oldMember, newMember)=>{
     ...
     if(oldUserChannel === undefined && newUserChannel !== undefined) {
       ...
       client.channels.get(config.voicelog).send(`**${newMember.user.tag}** (${newMember.user.id}) joined the channel: **${newMember.voiceChannel.name}** at ${moment().format('LLLL')}`)

       } else if(newUserChannel === undefined){

       client.channels.get(config.voicelog).send(`**${newMember.user.tag}** (${newMember.user.id}) left the channel: **${oldMember.voiceChannel.name}** at ${moment().format('LLLL')}`)
  }
})

/**VOICECHANNEL SWITCH DETECTION -> work in progress
client.on('voiceStateUpdate', (oldmember, newmember)=>{
      ...
      if (oldvoice && newvoice && oldvoice.id != newvoice.id) channelStatus = 'Moved'
            ...
            client.channels.get(config.voicelog).send("Switched from:", oldmember.voiceChannel, "to", newmember.voiceChannel)
 })
**/
